...
/** given the current accumulated set of personae and the current parsing state
  ...
  */
  def parsePersonae(acc: Accumulator, line: String): Accumulator = {
    acc match {
      case (characters,Other) => if (isDramatisStart(line)) (characters, Dramatis)  else acc
      case (characters,Dramatis) => if (isDramatisEnd(line)) {
        (characters, Other)
      } else {
        val accumulatorOpt = parsePersona(line).map(p => (characters + p, Dramatis))

        accumulatorOpt.getOrElse(acc)
      }
    }
  }
